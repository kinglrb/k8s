...
kubelet
运行在每个节点，主要的“节点代理”，
每个节点都会启动kubelet进程，用来处理Master节点下发到本节点的任务，
按照PodSpec描述，管理Pod和其中的容器
PodSpec： #描述 pod 的 YAML 或者 JSON (文件)对象

kubelet 通过各种机制（主要通过 apiserver ）获取 PodSpec, 保证 PodSpec 中描述的容器健康运行
...
--image-service-endpoint string                          #[实验]远程镜像服务端点。如果没有指定，默认情况下将与容器运行时端点相同。目前在 Linux 上支持unix 套接字，在 windows 上支持 tcp 。  例如:'unix:///var/run/dockershim.sock', 'tcp://localhost:3735'
...
--kubeconfig string                                      #kubeconfig文件路径，用来指定如何连接到 API server ，此时将使用 --api-servers  除非设置了--require-kubeconfig（默认“/var/lib/kubelet/kubeconfig”）
